...
本项目与目前现有利用 MSU-1 重建的 BS Zelda 改版作品有着本质性的区别。因卫星广播数据下发在 PSRAM 中的代码近乎永久性物理丢失，利用 MSU-1 补丁 + 音视频资源只能将目前存在的 DumpROM 重建为一部“看起来很相似”的独立游戏。而利用本项目的功能，可以近乎于重建当年 Satellaview 的完整逻辑和体验。如果说 MSU-1 改版是补完了游戏部分的缺失，那本项目能做到的事情就是带你真正的回到 1995 年的那个晚上。

## 🛠️ 核心功能
* **自动修改配置**：开启程序后可自动完成模拟器 BS-X 部分的相关配置。
* **虚拟卫星推送**：模拟 18:00 广播开始后，自动触发数据下发。
* **视频遮罩投影**：在模拟器窗口上方自动叠加剧情视频与等待画面，模拟重构丢失的 PSRAM 部分程序。
...
1. **配置模拟器**：
   ...
   * 下载配套使用的汉化资源包，解压，将 `bszelda` 文件夹放置在 Mesen 模拟器的根目录中。

2. **启动程序**：
   * 运行 `main.py`或 Satellaview-Terminal.exe 。
...
3. **注意事项**：
   * 在观看开场以及结局视频时，请勿操作模拟器的手柄或键盘按键，以免影响 Lua 脚本的同步运行。（实际上在 1995 年的原版程序中你也没办法按键。）
...
* **UI部分显示不全或错乱**
  * 由于缺少4K显示器进行测试，故无法完善此部分，建议将 Windows 缩放比例调整为 150% 即可正常使用。
